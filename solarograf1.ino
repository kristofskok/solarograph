...
void loop() {
  if (digitalRead(vklop) == LOW)  {//on off stikalo
    ...
    if (t.sec == 0 && smer==true) { // Zaklop zaprt, ali ga naj odprem?
        if ( t.hour == 7 && t.min == 43 ) { // zjutraj za ospredje
            ekspozicija = 17*60; 
            gibanje = true;
            smer = false;
        }
        else if ( t.hour == 16 && t.min == 0 ) { // zvecer za ospredje
            ekspozicija = 12*60; 
            gibanje = true;
            smer = false;
        }
        else if ( t.hour > 8 && t.hour < 15 ) { // slikanje Sonca
    ...
    if (!gibanje && smer == false && ((rtc.getUnixTime(t) - cas) >= ekspozicija)) { // zaklop odprt za ekspozicijski cas
      gibanje = true; // zaklop se premika
      smer = true; // zapiranje
      #ifdef DEBUG
